...
import UniversityLogoWall from '../components/UniversityLogoWall';
import { ArrowRight, CheckCircle, MessageCircle, Star, Globe, GraduationCap, Users, Award, Plane } from 'lucide-react';
import { countries } from '../data';

const services = [
  { icon: <Users size={28} />, title: 'Student Counseling', desc: 'Personalized guidance matching your academic background, goals and budget to the right country and university.', color: 'bg-blue-100 text-blue-600' },
  { icon: <GraduationCap size={28} />, title: 'Admission Processing', desc: 'We handle every form, every deadline — from SOP writing to document attestation and offer letter follow-up.', color: 'bg-amber-100 text-amber-600' },
  ...
  { icon: <Globe size={28} />, title: 'Scholarship Assistance', desc: 'We actively find and apply for scholarships matching your profile — GKS, Erasmus+, Malaysian, and more.', color: 'bg-sky-100 text-sky-600' },
  ...
  return (
    <div className="overflow-hidden">
      ...
      <section className="py-20 bg-white border-y border-slate-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {[
              { end: 500, suffix: '+', label: 'Students Placed', icon: <GraduationCap size={24} /> },
              { end: 98, suffix: '%', label: 'Visa Approval Rate', icon: <Award size={24} /> },
              { end: 10, suffix: '+', label: 'Countries', icon: <Globe size={24} /> },
              { end: 3, suffix: '', label: 'Office Locations', icon: <Plane size={24} /> },
            ].map((stat, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} viewport={{ once: true }}
                className="rounded-3xl bg-sky-50 p-8 text-center shadow-sm border border-sky-100">
                ...
                <div className="text-4xl font-extrabold text-blue-600 mb-1">
                  <Counter end={stat.end} suffix={stat.suffix} label="" />
                </div>
      ...
      <section className="py-24 bg-gradient-to-b from-sky-50 to-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <span className="inline-block px-4 py-1.5 rounded-full bg-blue-100 text-blue-600 font-bold uppercase tracking-widest text-xs mb-4">Where Will You Go?</span>
            <h2 className="text-4xl md:text-5xl font-extrabold text-slate-900 mt-2 mb-4">Study <span className="text-gradient-blue">Destinations</span></h2>
              ...
              <motion.div key={country.id} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} viewport={{ once: true }}
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6">
                  ...
                  <Link to={`/country/${country.id}`}
                    className="inline-flex items-center gap-2 bg-white/20 backdrop-blur-sm border border-white/30 text-white text-sm font-semibold px-4 py-2 rounded-full hover:bg-blue-600 hover:border-blue-600 transition-all group-hover:translate-x-1">
      ...
      <section className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <span className="inline-block px-4 py-1.5 rounded-full bg-blue-100 text-blue-600 font-bold uppercase tracking-widest text-xs mb-4">What We Do</span>
            <h2 className="text-4xl md:text-5xl font-extrabold text-slate-900 mt-2 mb-4">Our <span className="text-gradient-blue">Services</span></h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {services.map((s, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} viewport={{ once: true }}
                whileHover={{ y: -4 }}
                className="bg-white rounded-3xl p-8 shadow-sm hover:shadow-xl transition-all border border-slate-100 hover:border-blue-100 group">
                ...
                <Link to="/services" className="inline-flex items-center gap-1 text-blue-600 font-semibold text-sm hover:text-sky-500 transition-colors group-hover:gap-2">
                  Learn more <ArrowRight size={14} />
                </Link>
              </motion.div>
            ))}
          ...
          <div className="text-center mt-12">
            <Link to="/services"
              className="inline-flex items-center gap-2 bg-blue-600 text-white px-8 py-4 rounded-full font-bold hover:bg-blue-700 transition-all shadow-lg hover:shadow-xl hover:scale-105">
      ...
      <section className="py-24 bg-gradient-to-br from-sky-50 to-white relative overflow-hidden">
        ...
        <div className="absolute -bottom-20 -left-20 w-[300px] h-[300px] rounded-full bg-sky-100/60 blur-2xl" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <motion.div initial={{ opacity: 0, x: -40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              ...
              <h2 className="text-4xl md:text-5xl font-extrabold text-slate-900 mb-6 leading-tight">
                Founded by someone who <span className="text-blue-600">lived the journey.</span>
              </h2>
              ...
              <div className="mt-10 flex flex-col sm:flex-row gap-4">
                <a href="https://forms.gle/grR8xEBQG9rUCmjV7" target="_blank" rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-bold px-8 py-4 rounded-full transition-all hover:scale-105 hover:shadow-xl">
                  Start Your Journey <ArrowRight size={18} />
                </a>
                <Link to="/about"
                  className="inline-flex items-center gap-2 bg-white border border-slate-200 hover:bg-slate-50 text-slate-800 font-bold px-8 py-4 rounded-full transition-all">
            ...
            <motion.div initial={{ opacity: 0, x: 40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="grid grid-cols-2 gap-6">
              {[
                ...
                <motion.div key={i} initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.1 }} viewport={{ once: true }}
                  whileHover={{ y: -4 }}
                  className="bg-white border border-slate-200 shadow-sm rounded-3xl p-6 text-center hover:shadow-md hover:border-blue-200 transition-all">
                  <div className="text-4xl font-extrabold text-slate-900 mb-1">{stat.value}</div>
                  <div className="text-blue-600 font-bold text-sm uppercase tracking-wider">{stat.label}</div>
      ...
      <section className="py-24 bg-sky-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <span className="inline-block px-4 py-1.5 rounded-full bg-blue-100 text-blue-600 font-bold uppercase tracking-widest text-xs mb-4">Student Reviews</span>
            <h2 className="text-4xl md:text-5xl font-extrabold text-slate-900 mt-2 mb-4">What Our <span className="text-gradient-blue">Students Say</span></h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {testimonials.map((t, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.15 }} viewport={{ once: true }}
                whileHover={{ y: -4 }}
                className="bg-white rounded-3xl p-8 border border-slate-100 hover:shadow-xl hover:border-blue-100 transition-all relative">
                <div className="text-5xl text-slate-100 font-serif absolute top-6 right-8">"</div>
          ...
          <div className="text-center mt-12">
            <Link to="/success-stories"
              className="inline-flex items-center gap-2 text-blue-600 font-bold hover:text-sky-500 transition-colors">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <p className="text-blue-100 text-xl mb-10 max-w-2xl mx-auto">
              Book a free consultation. No fees, no obligations — just honest, expert guidance from people who've been there.
            </p>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <motion.a
                href="https://forms.gle/grR8xEBQG9rUCmjV7" target="_blank" rel="noopener noreferrer"
                className="inline-flex items-center gap-2 bg-white text-blue-700 font-extrabold px-10 py-5 rounded-full hover:bg-amber-400 hover:text-white shadow-xl hover:shadow-2xl transition-all"
              ...
              <a href="https://wa.me/8801941646278" target="_blank" rel="noopener noreferrer"
                className="inline-flex items-center gap-2 bg-white/20 border-2 border-white/40 text-white font-bold px-8 py-4 rounded-full hover:bg-white/30 transition-all">
      ...
      <section id="contact" className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            <motion.div initial={{ opacity: 0, x: -40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              <span className="inline-block px-4 py-1.5 rounded-full bg-blue-100 text-blue-600 font-bold uppercase tracking-widest text-xs mb-4">Get In Touch</span>
              ...
              <div className="flex flex-col sm:flex-row gap-4 mt-10">
                ...
                <a href="https://www.facebook.com/share/187gaTjhFD/" target="_blank" rel="noopener noreferrer"
                  className="inline-flex items-center justify-center gap-2 bg-blue-600 hover:bg-sky-600 text-white font-bold px-6 py-4 rounded-full transition-all hover:scale-105 hover:shadow-xl">
                  Facebook Page
                </a>
              </div>
            </motion.div>
